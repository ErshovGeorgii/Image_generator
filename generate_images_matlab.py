...
def img_generate(noize, sigma):
    ...
    for k in range(n):
        ...
        for i in range(-max_dist, max_dist):
            for j in range(-max_dist, max_dist):
                if x + i > 1 and x + i < 1024 and y + j > 1 and y + j < 1024:
    ...
    return (img, [image, stars], [image, circles])
...
noise_step = 0.05
sigma_step = 0.5
true_path = "/home/alex/Desktop/Gosha/my_data"
test_path = "/home/alex/Desktop/Gosha/my_data_pricol"
...
for s_i in range(1, 7):
    ...
    for n_i in range(1, 11):
        ...
        path = "{}/sigma_{}/noise_{}".format(true_path, sigma, noise)
        try:
            os.makedirs(path)
        except: 
            pass
        ...
        for i in range(n):
            ...
            one_image[0].save("{}/sigma_{}/noise_{}/image_{}_{}_{}.jpg".format(true_path, sigma, noise, sigma, noise, i))
            for j in range(len(one_image[1][1])):
                data_train.append(["image_{}_{}_{}".format(sigma, noise, i), j, one_image[1][1][j][0], one_image[1][1][j][1], round(one_image[1][1][j][2], 2), one_image[1][1][j][3]])
            for k in range(len(one_image[2][1])):
                data_train.append(["image_{}_{}_{}".format(sigma, noise, i), k, one_image[2][1][k][0], one_image[2][1][k][1], round(one_image[2][1][k][2], 2), one_image[2][1][k][3]])
            

#        pd.DataFrame(data_train).to_csv("{}/data_sigma_{}_noise_{}.csv".format(path, sigma, noise))
        df = pd.DataFrame(data_train)
        df.to_csv("{}/data_sigma_{}_noise_{}.csv".format(path, sigma, noise))
        break
    break